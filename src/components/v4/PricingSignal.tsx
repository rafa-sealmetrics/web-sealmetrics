...
export function PricingSignal({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="sig-pricing-page">
      ...
      <section className="sig-pricing-traffic"><div className="sig-pricing-section-head"><div><p className="sig-pricing-tag">{t.trafficTag}</p><h2>{t.trafficTitle}</h2></div><p>{t.trafficBody}</p></div><div className="sig-pricing-table-wrap"><table><thead><tr>{t.trafficHeaders.map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{t.trafficRows.map(row=><tr key={row[0]}>{row.map((cell,i)=>i===0?<th scope="row" key={cell}>{cell}</th>:<td key={cell}>{cell}</td>)}</tr>)}</tbody></table></div></section>

      <section className="sig-pricing-comparison"><div className="sig-pricing-section-head"><div><p className="sig-pricing-tag">{t.comparisonTag}</p><h2>{t.comparisonTitle}</h2></div><p>{t.comparisonBody}</p></div><div className="sig-pricing-table-wrap sig-pricing-comparison-table"><table><thead><tr><th>{t.feature}</th><th>{t.agentic}<small>{t.free}</small></th><th>{t.growth}</th><th>{t.scale}</th><th>{t.enterprise}</th></tr></thead><tbody>{comparison.map(section=><Fragment key={section.category[0]}><tr className="sig-pricing-category"><th colSpan={5}>{section.category[locale==="es"?1:0]}</th></tr>{section.rows.map(([label,a,g,s,e])=><tr key={label[0]}><th scope="row">{label[locale==="es"?1:0]}</th><td><Cell value={a} locale={locale}/></td><td><Cell value={g} locale={locale}/></td><td><Cell value={s} locale={locale}/></td><td><Cell value={e} locale={locale}/></td></tr>)}</Fragment>)}</tbody></table></div></section>
